...
add_action('rest_api_init', 'reviewnews_register_plugins_routes');
function reviewnews_register_plugins_routes()
{
  register_rest_route('aft-useful-plugins/v1', '/get-useful-plugins', array(
    'methods' => 'GET',
    'callback' => 'reviewnews_get_all_useful_plugins',
...
function reviewnews_get_all_useful_plugins(\WP_REST_Request $request)
{
  ...
  foreach ($plugin_array as $plugin) {
    ...
    $button_text = __('Install Now', 'reviewnews');
    ...
    if (!is_wp_error($api)) { // confirm error free
      ...
      $main_plugin_file = reviewnews_get_plugin_file($plugin['slug']); // Get main plugin file
      ...
      if (reviewnews_check_file_extension($main_plugin_file)) { // check file extension
        if (is_plugin_active($main_plugin_file)) {
          // plugin activation, confirmed!
          $button_classes = 'button disabled';
          $button_text = __('Activated', 'reviewnews');
        } else {
          // It's installed, let's activate it
          $button_classes = 'activate button button-primary';
          $button_text = __('Activate', 'reviewnews');
        }
      }
      $data['plugins'][] = reviewnews_render_plugin_lists_template($plugin, $api, $button_text, $button_classes, $plugin_description, $title);
    }
  }
...
function reviewnews_render_plugin_lists_template($plugin, $api, $button_text, $button_classes, $plugin_description, $title)
{
  ...
  <div class="aft-plugin-installer">
    <div class="plugin">
      ...
      <div class="plugin-info">
        ...
        <p class="plugin-author"><?php _e('By', 'reviewnews'); ?> <?php echo $api->author; ?></p>
      </div>
      <ul class="activation-row">
        <li>
          <?php if ($api->slug == 'templatespare' && $button_text == 'Activated') { ?>
            <a class="button-primary" href="<?php echo site_url() . '/wp-admin/admin.php?page=starter-sites' ?>"><?php echo _e('Get Starter Sites', 'reviewnews') ?></a>
        ...
        <li>
          <a href="https://wordpress.org/plugins/<?php echo $api->slug; ?>/" target="_blank">
            <?php _e('More Details', 'reviewnews'); ?>
          </a>
        </li>
      </ul>
    </div>
...
function reviewnews_get_plugin_file($plugin_slug)
{
...
function reviewnews_check_file_extension($filename)
{
